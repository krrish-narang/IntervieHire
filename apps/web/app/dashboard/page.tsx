...
function getSessionTranscriptText(transcript: unknown) {
  if (!Array.isArray(transcript)) return '';

  return transcript
    .filter((entry: any) =>
      typeof entry?.text === 'string' &&
      (entry?.speaker === 'candidate' || entry?.speaker === 'ai' || entry?.type === 'speech_to_text_transcript'),
    )
    .map((entry: any) => {
      if (entry.speaker === 'candidate' || entry.speaker === 'ai') {
              ...
              {reportItems.length ? (
                reportItems.map((session: any) => (
                  <div key={session.id} className="rounded-2xl border bg-slate-50 p-4 text-sm">
                    ...
                    {getSessionTranscriptText(session.transcript) ? (
                      <details className="mt-3 rounded-xl bg-white p-3">
                        <summary className="cursor-pointer font-semibold text-slate-700">Interview transcript</summary>
                        <p className="mt-2 max-h-32 overflow-auto whitespace-pre-wrap text-xs leading-5 text-slate-600">
                          {getSessionTranscriptText(session.transcript)}
                        </p>
                      </details>
                    ) : null}
