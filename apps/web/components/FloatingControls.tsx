...
export function FloatingControls() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((current) => [
        ...current,
        { role: 'assistant', content: 'I could not reach the DeepSeek assistant right now. Check DEEPSEEK_API_KEY and try again.' },
      ]);
    } finally {
      setLoading(false);
    }
      ...
      {open ? (
        <div className="fixed bottom-20 left-4 z-50 w-[min(92vw,22rem)] overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-2xl shadow-slate-300/60">
          <div className="flex items-center justify-between border-b px-4 py-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">DeepSeek assistant</p>
